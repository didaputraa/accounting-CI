...
$(document).ready(function(){
	...
	$("#simpan").click(function(){
		...
		var tanggal_production 	= $("#tanggal_production").val();
		...
		var string = "id_id="+id_id+"&po_number="+po_number+"&bukti_pengeluaran_no="+bukti_pengeluaran_no+"&tanggal_production="+tanggal_production+"&kode_barang="+kode_barang+"&nama_barang="+nama_barang+"&satuan="+satuan+"&digunakan="+digunakan+"&disubkontrakkan="+disubkontrakkan+"&penerima_subkontrak="+penerima_subkontrak;
		...
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/production/simpan",
			data	: string,
			cache	: false,
			success	: function(data){
				$.messager.show({
	...
	function tampil_data(){
		var po_number = $("#id_production").val();
		var string = "id_production="+id_production;
		
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/id_production/DetailProduction",
			data	: string,
			cache	: false,
			success	: function(data){
	...
	$("#kembali").click(function(){
		window.location.assign('<?php echo base_url();?>index.php/production');
	});
...
function editData(id){
	...
	$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/production/edit",
			data	: string,
			cache	: true,
			dataType : "json",
			success	: function(data){
				...
				$("#tanggal_production").val(data.tanggal_production);
	...
	function tampil_data(){
		var po_number = $("#po_number").val();
		var string = "po_number="+id_production;
		
		$.ajax({
			type	: 'POST',
			url		: "<?php echo site_url(); ?>/production/DetailProduction",
			data	: string,
			cache	: false,
			success	: function(data){
				$("#tampil_data").html(data);
...
<button type="button" name="download" id="download" onclick="download_data();" class="easyui-linkbutton" data-options="iconCls:'icon-save '">Download</button>

<a href="<?php echo base_url();?>index.php/production">
<button type="button" name="refresh" id="refresh" class="easyui-linkbutton" data-options="iconCls:'icon-reload'">Refresh</button>
</a>
</div>
<div style="float:right; padding-bottom:5px;">
<form name="form" method="post" action="<?php echo base_url();?>index.php/production">
	...
	if($data->num_rows()>0){
    	...
    	<tr>
           ...
           <td align="right" width="100" ><?php echo number_format($db['debet']); ?></td>
            ...
            <?php echo "<a href='javascript:editData(\"{$db['id_production']}\")'>";?>
			<img src="<?php echo base_url();?>asset/images/ed.png" title='Edit'>
			</a>
            <a href="<?php echo base_url();?>index.php/production/delete/<?php echo $db['id_production'];?>"
...
<tr>    
            <td>Tanggal Production</td>
            <td>:</td>
            <td>
            <input type="text" name="tanggal_production" id="tanggal_production" size="15" maxlength="15" />
            </td>
        </tr> 
<tr>    
	<td>Kode Barang</td>
...
<tr>    
	<td>Digunakan</td>
    <td>:</td>
    <td><input type="text" name="digunakan" id="digunakan"  size="50" maxlength="50" /></td>
</tr>
<tr>    
